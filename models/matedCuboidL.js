...
export function createMatedCuboidL(
  ...
  // Get the specified face from FACES
  const cuboidFace = FACES[cuboidFaceName];
  if (!cuboidFace) {
    throw new Error(`Invalid cuboid face: ${cuboidFaceName}. Valid options are: ${Object.keys(FACES).join(', ')}`);
  }
  
  // Get the specified face from FACES for L-profile
  const lProfileFace = FACES[lProfileFaceName];
  if (!lProfileFace) {
    throw new Error(`Invalid L-profile face: ${lProfileFaceName}. Valid options are: ${Object.keys(FACES).join(', ')}`);
  }
